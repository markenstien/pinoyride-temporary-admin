-- For riders with more than one non-deleted 'pr-user-wallet' row (type =
-- 'pr-user-wallet', user_type = 'rider'), keep only the most recently
-- created one and soft-delete the rest, along with that wallet's
-- wallet_history ledger rows.
--
-- Companion script for the 'user-wallet' type is
-- delete_duplicate_user_wallets.sql — the two types are kept in
...
-- ---------------------------------------------------------------
DROP TABLE IF EXISTS _dup_pr_user_wallet_losers;
CREATE TEMP TABLE _dup_pr_user_wallet_losers AS
WITH ranked AS (
  SELECT
    w.id,
...
LEFT JOIN _dup_pr_user_wallet_losers dl ON dl.id = w.id
LEFT JOIN LATERAL (
...
WHERE w.user_type = 'rider'
  ...
  AND w.user_id IN (
    ...
    JOIN _dup_pr_user_wallet_losers dl2 ON dl2.id = w2.id
  )
...
SET deleted_at = NOW(),
    updated_at = NOW()
WHERE wallet_id IN (SELECT id FROM _dup_pr_user_wallet_losers)
...
SET deleted_at = NOW(),
    updated_at = NOW()
WHERE id IN (SELECT id FROM _dup_pr_user_wallet_losers);
